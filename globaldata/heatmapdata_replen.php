...
FROM
    gillingham.vectormap
        LEFT JOIN
    gillingham.replen_hist ON replen_bay = BAY
WHERE
  CSLS = 'LSE' "
...
FROM
    ...
    gillingham.replen_hist ON  replen_bay = BAY
WHERE
    VECTWHSE = $var_whse and CSLS = 'CSE' and replen_date = '$var_datesel' 
GROUP BY BAY");  //$orderby pulled from: include 'slopecat_switch_orderby.php';
    $baycolor->execute();
    $baycolorarray = $baycolor->fetchAll(pdo::FETCH_ASSOC);
}
...
if ($var_zone == 'L%') {
    $loosetext = $conn1->prepare("SELECT 
                            ...
                            WHERE
                                WHSE = $var_whse and TEXT not like '%Picks%'");  //$orderby pulled from: include 'slopecat_switch_orderby.php';
    $loosetext->execute();
