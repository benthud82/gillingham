...
do {
    ...
    if ($flow_totalcube > $cap_flow) {
        //issue with not meeting flow cube because some bin locations are larger than flow and takes nextgrid of bin and frees up flow space.
        //cannot delete all nextgrid location types as flow, just need to skip temporarily until 
        //$flowloop += 1;
...
//        $querydelete2->execute();
        ...
        $sqlflowexclude = " and nextgrid_loctype <> 'FLOW' ";
    }
    ...
    if ($bin_totalcube > $cap_bb && $binloop == 0) {
        ...
        $sqldelete4 = "DELETE
                                ...
                                FROM
                                    ...
                                    gillingham.rpc_reductions ON itemtf_item = rpc_item
                                        AND itemtf_grid = rpc_grid
                                        WHERE (rpc_item is null or itemtf_loctype = 'BIN')";
        $querydelete4 = $conn1->prepare($sqldelete4);
        $querydelete4->execute();
...
//pull in top item based of next grid flag
    ...
    if (!isset($array_topitem[0]['nextgrid_rpcdecrease'])) {
        break;
    }
...
//how much capacity is now used
    ...
    if($bin_totalcube > $cap_bb && $flow_totalcube > $cap_flow ){
        break;
    }
