...
$sqldelete = "TRUNCATE gillingham.gill_grouped_cse";
$querydelete = $conn1->prepare($sqldelete);
$querydelete->execute();

$sqldelete2 = "TRUNCATE gillingham.gill_raw_30day_cse";
$querydelete2 = $conn1->prepare($sqldelete2);
$querydelete2->execute();


//create 30 day table
$sql_30day = $conn1->prepare("INSERT into gillingham.gill_raw_30day_cse (idsales, ITEM, PKGU, PKTYPE, UNITS, PICKDATE, LOCATION)
                                                                SELECT 
                                                                    idGill_Test,
                                                                    ITEM,
                                                                    1,
...
$rawsql = $conn1->prepare("SELECT 
                                                    ...
                                                    FROM
                                                        gillingham.gill_raw_30day_cse A
                                                            JOIN
...
do {
    ...
    $sql = "INSERT IGNORE INTO gillingham.gill_grouped_cse ($columns) VALUES $values";
    $query = $conn1->prepare($sql);
    $query->execute();
    $maxrange += 20000;
} while ($counter <= $rowcount); //end of item by whse loop
//Truncate NPTLSD file
$sqldelete = "TRUNCATE gillingham.nptsld_cse";
$querydelete = $conn1->prepare($sqldelete);
$querydelete->execute();

//Create NPTSLD file from gill_grouped
$sql2 = "INSERT IGNORE INTO gillingham.nptsld_cse
                         SELECT 
    A.GROUPED_ITEM,
    1,
    A.GROUPED_PKTYPE,
...
//no sales update
$sql = "INSERT IGNORE into gillingham.nptsld_cse
                        SELECT 
                        ...
                        FROM
                            ...
                            gillingham.item_master X ON X.ITEM = A.slotmaster_item
                                LEFT JOIN
                            gillingham.nptsld_cse B ON slotmaster_item = B.ITEM
                                AND PKTYPE = slotmaster_pkgu
                        WHERE
                            B.ITEM IS NULL
                                AND slotmaster_loc >= '70*'
